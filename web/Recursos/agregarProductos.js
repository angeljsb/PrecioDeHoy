...
const paginacion = {
    ...
    setPagina: (number) => {
        ...
        const event = new Event("changeproducts");
        contenedorListaProductos.dispatchEvent(event);
    }
};
...
formularioProductos.addEventListener('submit', guardar);
...
contenedorListaProductos.addEventListener('changeproducts', (e) => {
    ...
    paginacion.productosMostrados.forEach((producto) => {
        ...
        if (producto.precio_dolares && producto.precio_dolares !== "null") {
            htmlCompleto += `<div><table class="ph-container--small-y">
                        <tbody>
                            <tr>
    ...
    if(productos.length > paginacion.productosPorPagina){
        htmlCompleto += `
    ...
    const prev = contenedorListaProductos.querySelector("#prev-page");
    const next = contenedorListaProductos.querySelector("#next-page");

    if(prev && next){
        const changePage = (quantity) => {
            paginacion.setPagina(paginacion.pagina + quantity);
        };
